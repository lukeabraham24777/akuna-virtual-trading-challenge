...
class MarketMaker:
    ...
    POS_CAP = 34.0         # per-option position cap (scaled by cash)
    ...
    RFQ_LOCK_TH = 0.018
    RFQ_UNLOCK = 0.006
    GROSS_CAP = 80.0       # sens-weighted gross book size before throttling
    ...
    def __init__(
        ...
        # solvency governor: exact replica of the autograder's reserve balance
        # (max-loss debited per trade, gross-leg credits at expiry); used ONLY
        # as a veto layer -- strategy decisions are otherwise unchanged
        self._hard = float(cash_balance)
        self._gross: dict[int, list] = {}
        self._day = 0
        self._pv = 0
    ...
    def _gov_avail(self) -> float:
        # true reserve above an untouchable buffer; end-of-day solvency can
        # never be breached while every commitment fits inside this
        return self._hard - max(0.35, 0.035 * self._cash0)
    ...
    def _quote_inner(self, option: BinaryOption, counterparty_id: int) -> Quote:
        ...
        # solvency governor: a filled bid consumes bid*qty of true reserve, a
        # filled offer consumes (1-offer)*qty; sides that cannot afford one
        # contract fall back to the free boundary price
        gav = max(self._gov_avail(), 0.0)
        if bid > gav * 0.5:
            bid = 0.0
        if (1.0 - offer) > gav * 0.5:
            offer = 1.0
        if bid > 0.0:
            bq = int(max(1, min(bq, gav * 0.5 / max(bid, 0.005))))
        if offer < 1.0:
            oq = int(max(1, min(oq, gav * 0.5 / max(1.0 - offer, 0.005))))
    ...
    def _fok_inner(self, option: BinaryOption, fok_order: FokOrder) -> bool:
        ...
        headroom = self._headroom()
        # solvency governor: the full-fill max loss must fit the true reserve
        unit_ml = (1.0 - price) if we_sell else price
        if unit_ml * qty > max(self._gov_avail(), 0.0):
            return False
