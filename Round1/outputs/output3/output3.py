...
class MarketMaker:
    ...
    BASE_HALF = 0.014      # base half-spread added on top of uncertainty term
    UNC_HALF = 1.15        # half-spread contribution per unit of theo uncertainty
    HALF_MIN = 0.02
    HALF_MAX = 0.2
    TIGHT_MIN = 0.7       # global spread multiplier bounds (adaptive)
    TIGHT_MAX = 2.4
    SIZE_BASE = 12.0
    SIZE_MAX = 30.0
    FOK_FLOOR = 0.018      # minimum absolute edge for FOK acceptance
    FOK_UNC = 0.9         # extra edge required per unit of uncertainty
    FOKM_MIN = 0.75
    FOKM_MAX = 2.6
    TAIL_THEO = 0.07       # below this theo, selling requires the multiple rule
    TAIL_SELL_MULT = 1.6  # price must exceed this multiple of theo for tail shorts
    ...
    DILUTE_EDGE = 0.02    # accept tiny-edge FOKs from benign flow to dilute rivals
    RISK_MARGIN = 0.09     # fraction of initial cash kept as untouchable buffer
    POS_CAP = 26.0         # per-option position cap (scaled by cash)
    EXP_CAP = 11.0         # per-underlying probability-weighted exposure cap
    N_PRIOR = 32.0         # pseudo-observations credited to shrinkage priors
    ...
    WIN_HI = 0.6          # harvest-widen when winning more than this share
    HARVEST_CAP = 1.45
    DEF1_ADV = 0.025       # drawdown stages (fraction of cash), adverse-flow regime
    DEF2_ADV = 0.055
    DEF1_BEN = 0.05       # drawdown stages, benign regime
    DEF2_BEN = 0.1
    TOX_OVR_DAY = 6        # full sit-out when markouts this bad by this day
    TOX_OVR = 0.015
    TOX_OVR2_DAY = 5
    TOX_OVR2 = 0.01
    ...
    RFQ_LOCK_TH = 0.014
    RFQ_UNLOCK = 0.006
    GROSS_CAP = 55.0       # sens-weighted gross book size before throttling
