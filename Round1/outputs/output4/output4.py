...
class MarketMaker:
    ...
    BASE_HALF = 0.008      # base half-spread added on top of uncertainty term
    UNC_HALF = 0.6        # half-spread contribution per unit of theo uncertainty
    HALF_MIN = 0.01
    HALF_MAX = 0.12
    TIGHT_MIN = 0.45       # global spread multiplier bounds (adaptive)
    TIGHT_MAX = 2.4
    SIZE_BASE = 21.0
    SIZE_MAX = 60.0
    FOK_FLOOR = 0.009      # minimum absolute edge for FOK acceptance
    FOK_UNC = 0.55         # extra edge required per unit of uncertainty
    FOKM_MIN = 0.5
    FOKM_MAX = 2.6
    TAIL_THEO = 0.07       # below this theo, selling requires the multiple rule
    TAIL_SELL_MULT = 1.35  # price must exceed this multiple of theo for tail shorts
    LOTTERY_PRICE = 0.06   # cheap-option buys accepted at reduced edge (bounded loss)
    LOTTERY_EDGE = 0.008
    SKEW_G = 2.0
    ...
    DILUTE_EDGE = 0.003    # accept tiny-edge FOKs from benign flow to dilute rivals
    RISK_MARGIN = 0.06     # fraction of initial cash kept as untouchable buffer
    POS_CAP = 42.0         # per-option position cap (scaled by cash)
    EXP_CAP = 19.0         # per-underlying probability-weighted exposure cap
    N_PRIOR = 20.0         # pseudo-observations credited to shrinkage priors
    ...
    HARVEST_CAP = 1.25
    DEF1_ADV = 0.04       # drawdown stages (fraction of cash), adverse-flow regime
    DEF2_ADV = 0.09
    DEF1_BEN = 0.075       # drawdown stages, benign regime
    DEF2_BEN = 0.15
    ...
    RFQ_LOCK_TH = 0.024
    RFQ_UNLOCK = 0.006
    GROSS_CAP = 110.0       # sens-weighted gross book size before throttling
