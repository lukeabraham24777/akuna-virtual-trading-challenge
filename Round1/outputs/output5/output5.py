...
class MarketMaker:
    ...
    BASE_HALF = 0.03      # base half-spread added on top of uncertainty term
    UNC_HALF = 1.3        # half-spread contribution per unit of theo uncertainty
    HALF_MIN = 0.03
    HALF_MAX = 0.25
    ...
    SIZE_BASE = 7.0
    SIZE_MAX = 16.0
    FOK_FLOOR = 0.035      # minimum absolute edge for FOK acceptance
    FOK_UNC = 1.1         # extra edge required per unit of uncertainty
    FOKM_MIN = 1.0
    FOKM_MAX = 3.0
    TAIL_THEO = 0.07       # below this theo, selling requires the multiple rule
    TAIL_SELL_MULT = 1.7  # price must exceed this multiple of theo for tail shorts
    LOTTERY_PRICE = 0.04   # cheap-option buys accepted at reduced edge (bounded loss)
    LOTTERY_EDGE = 0.012
    SKEW_G = 2.0
    ...
    RISK_MARGIN = 0.12     # fraction of initial cash kept as untouchable buffer
    POS_CAP = 14.0         # per-option position cap (scaled by cash)
    EXP_CAP = 7.0         # per-underlying probability-weighted exposure cap
    N_PRIOR = 35.0         # pseudo-observations credited to shrinkage priors
    ...
    HARVEST_CAP = 1.35
    DEF1_ADV = 0.02       # drawdown stages (fraction of cash), adverse-flow regime
    DEF2_ADV = 0.045
    DEF1_BEN = 0.04       # drawdown stages, benign regime
    DEF2_BEN = 0.08
    TOX_OVR_DAY = 5        # full sit-out when markouts this bad by this day
    TOX_OVR = 0.012
    TOX_OVR2_DAY = 4
    TOX_OVR2 = 0.008
    RFQ_LOCK_DAY = 3       # earliest day the RFQ-channel lockdown may engage
    RFQ_LOCK_TH = 0.012
    RFQ_UNLOCK = 0.006
    GROSS_CAP = 30.0       # sens-weighted gross book size before throttling
