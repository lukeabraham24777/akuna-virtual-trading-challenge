...
class MarketMaker:
    ...
    BASE_HALF = 0.012      # base half-spread added on top of uncertainty term
    ...
    SIZE_BASE = 18.0
    SIZE_MAX = 40.0
    FOK_FLOOR = 0.02      # minimum absolute edge for FOK acceptance
    FOK_UNC = 0.7         # extra edge required per unit of uncertainty
    FOKM_MIN = 1.0
    FOKM_MAX = 1.0
    ...
    FADE_G = 0.0
    FADE_CAP = 0.035
    CP_TIGHT = 1.0        # spread multiplier for proven-benign counterparties
    DILUTE_EDGE = 9.0    # accept tiny-edge FOKs from benign flow to dilute rivals
    RISK_MARGIN = 0.06     # fraction of initial cash kept as untouchable buffer
    POS_CAP = 38.0         # per-option position cap (scaled by cash)
    EXP_CAP = 16.0         # per-underlying probability-weighted exposure cap
    ...
    CP_LOCK_TH = 9.0      # per-cp markout beyond which the cp is locked out
    CP_WIDEN_TH = 9.0    # per-cp markout beyond which quotes to it widen
    STEP_DD = 9.0        # step markout dollars (frac of cash) triggering fast widen
    WIN_LO = -1.0          # tighten when winning less than this share of RFQs
    WIN_HI = 2.0          # harvest-widen when winning more than this share
    HARVEST_CAP = 1.35
    DEF1_ADV = 0.1       # drawdown stages (fraction of cash), adverse-flow regime
    DEF2_ADV = 0.13
    DEF1_BEN = 0.1       # drawdown stages, benign regime
    DEF2_BEN = 0.13
    TOX_OVR_DAY = 9999        # full sit-out when markouts this bad by this day
    TOX_OVR = 0.02
    TOX_OVR2_DAY = 9999
    TOX_OVR2 = 0.012
    RFQ_LOCK_DAY = 9999       # earliest day the RFQ-channel lockdown may engage
    RFQ_LOCK_TH = 0.018
    RFQ_UNLOCK = 0.006
    GROSS_CAP = 100.0       # sens-weighted gross book size before throttling
    RSCALE_K = 0.0        # RFQ size shrink per unit of negative RFQ markout
    ...
    def __init__(
        ...
        self._cs = min(max(self._cash0 / 1000.0, 0.25), 4.0)
        self._day = 0
        self._pv = 0
    ...
    def _reserve(self) -> float:
        ...
        for oid, q in self.position.option_quantity_by_option_id.items():
            if q < 0 and oid not in self._settled:
                r += -q * 1.0
        return r
    ...
    def _headroom(self) -> float:
        return self._cash - self._reserve() - self.RISK_MARGIN * self._cash0
    ...
    def _step_advance_inner(self, new_underlying_state, new_option_state) -> None:
        ...
        if self._mark_rfq < -0.012:
            ...
            if self._win_ewma < self.WIN_LO:
                ...
                self._tight = min(self._tight * 1.035, self.HARVEST_CAP)  # harvest: widen while still winning
            if self._def_mode == 0:
        ...
        # drawdown circuit breaker (mark-to-model), stricter when flow is provably adverse
        mp_now = self._mark_pnl()
        if mp_now > self._peak:
            self._peak = mp_now
        dd = self._peak - mp_now
        adverse = self._g_mark < -0.008
        lim1 = (self.DEF1_ADV if adverse else self.DEF1_BEN) * self._cash0
        lim2 = (self.DEF2_ADV if adverse else self.DEF2_BEN) * self._cash0
        if dd > lim2:
            self._def_mode = 2
        elif dd > lim1:
            self._def_mode = 1
    ...
    def _quote_inner(self, option: BinaryOption, counterparty_id: int) -> Quote:
        ...
        if headroom < 0.02 * self._cash0 or self._def_mode >= 2 or self._rfq_lock:
            # lockdown: only strictly riskless prices (buy at 0.00 / sell at 1.00)
        ...
        buy_afford = max(headroom * 0.30, 0.0) / max(bid, 0.02)
        sell_afford = max(headroom * 0.30, 0.0) / max(1.0 - offer, 0.02)
        ...
        gross = 0.0
        for oid2, q2 in self.position.option_quantity_by_option_id.items():
            if q2 and oid2 not in self._settled:
                o2 = self._opt_by_id.get(oid2)
                if o2 is not None:
                    _, _, s2 = self._theo3(o2)
                    gross += abs(q2) * s2
        gscale = self._clamp(1.25 - gross / (self.GROSS_CAP * self._cs), 0.35, 1.0)
        rscale = self._clamp(1.0 + self._mark_rfq * self.RSCALE_K, 0.12, 1.0) if self._mark_rfq < 0 else 1.0
        bq = int(max(1, min(base * bid_mult * buy_scale * gscale * rscale, buy_afford, self.SIZE_MAX * self._cs)))
        oq = int(max(1, min(base * off_mult * sell_scale * gscale * rscale, sell_afford, self.SIZE_MAX * self._cs)))
    ...
    def _fok_inner(self, option: BinaryOption, fok_order: FokOrder) -> bool:
        ...
        # hard risk checks assuming full fill
        if we_sell:
            if pos < 0:
                add_liab = qty * 1.0
            elif pos - qty < 0:
                add_liab = (qty - pos) * 1.0
            else:
                add_liab = 0.0
            if headroom - add_liab + price * qty < 0:
                return False
            if p_raw < self.TAIL_THEO:
                if price < min(self.TAIL_SELL_MULT * max(p_raw, 0.01), p_raw + 0.25):
                    if not reduces:
                        return False
        else:
            cost = price * qty
            if cost > max(headroom, 0.0) * 0.6:
                return False
            if price <= self.LOTTERY_PRICE and edge >= self.LOTTERY_EDGE and headroom > cost * 3:
